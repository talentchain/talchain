...
public class Utils {
    ...
    public static byte[] parseVarData(String data){
        if (data == null || data.equals("")) return ByteUtil.EMPTY_BYTE_ARRAY;
        if (data.startsWith("0x")) {
            data = data.substring(2);
            if (data.equals("")) return ByteUtil.EMPTY_BYTE_ARRAY;
    ...
    public static byte[] parseNumericData(String data){
        ...
        if (data == null || data.equals("")) return ByteUtil.EMPTY_BYTE_ARRAY;
        byte[] dataB = org.talchain.util.Utils.unifiedNumericToBigInteger(data).toByteArray();
