package org.talchain.db;

import org.talchain.datasource.QuotientFilter;
...
public class QuotientFilterTest {
    ...
    public void perfTest() {
        ...
        for (int i = 0; i < 5_000_000; i++) {
            f.insert(HashUtil.sha3(intToBytes(i)));

            // inserting duplicate slows down significantly
            if (i % 10 == 0) f.insert(HashUtil.sha3(intToBytes(0)));

            if (i > 100_000 && i % 2 == 0) {
                f.remove(HashUtil.sha3(intToBytes(i - 100_000)));
            }
            if (i % 10000 == 0) {
