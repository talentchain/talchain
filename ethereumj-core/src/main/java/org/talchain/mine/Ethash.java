...
import static org.talchain.mine.MinerIfc.MiningResult;
...
public class Ethash {
    ...
    private  Pair<byte[], byte[]> hashimotoLight(BlockHeader header, byte[] nonce) {
        return getEthashAlgo().hashimotoLight(getFullSize(), getCacheLight(),
                HashUtil.sha3(header.getEncodedWithoutNonce()), nonce);
    }
    ...
    public Pair<byte[], byte[]> hashimotoFull(BlockHeader header, long nonce) {
        return getEthashAlgo().hashimotoFull(getFullSize(), getFullDataset(), HashUtil.sha3(header.getEncodedWithoutNonce()),
                longToBytes(nonce));
    }
