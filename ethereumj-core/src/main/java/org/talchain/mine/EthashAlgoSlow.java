...
//        if (nonce.length != 4) throw new RuntimeException("nonce.length != 4");
        ...
        byte[] s = HashUtil.sha512(merge(blockHeaderTruncHash, reverse(nonce)));
        ...
        return Pair.of(cmix, HashUtil.sha3(merge(s, cmix)));
    }
