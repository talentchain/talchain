...
//        this.repository = blockchain.getRepository();
        ...
        if (!infos.isEmpty()) {
            for (TransactionInfo info : infos) {
                ...
                if (txBlock.isEqual(blockStore.getChainBlockByNumber(txBlock.getNumber()))) {
                    ...
                    fireTxUpdate(info.getReceipt(), EthereumListener.PendingTransactionState.INCLUDED, txBlock);
                    return;
                }
            }
        }
    ...
    private void fireTxUpdate(TransactionReceipt txReceipt, EthereumListener.PendingTransactionState state, Block block) {
    ...
    private boolean addPendingTransactionImpl(final Transaction tx) {
        ...
        if (!txReceipt.isValid()) {
            fireTxUpdate(txReceipt, EthereumListener.PendingTransactionState.DROPPED, getBestBlock());
        } else {
            pendingTransactions.add(new PendingTransaction(tx, getBestBlock().getNumber()));
            fireTxUpdate(txReceipt, EthereumListener.PendingTransactionState.NEW_PENDING, getBestBlock());
        }
        ...
        for (int i = 0; i < block.getTransactionsList().size(); i++) {
            ...
            if (pendingTransactions.remove(pend)) {
                try {
                    ...
                    fireTxUpdate(receipt, EthereumListener.PendingTransactionState.INCLUDED, block);
                } catch (Exception e) {
    ...
    private void updateState(Block block) {
        ...
        for (PendingTransaction tx : pendingTransactions) {
            ...
            fireTxUpdate(receipt, EthereumListener.PendingTransactionState.PENDING, block);
        }
    }
