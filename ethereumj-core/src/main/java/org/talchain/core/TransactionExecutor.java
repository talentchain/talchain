...
import org.talchain.util.ByteArraySet;
import org.ethereum.vm.*;
import org.talchain.vm.program.Program;
import org.talchain.vm.program.ProgramResult;
...
import org.talchain.util.BIUtil;
import org.talchain.vm.*;
...
import static org.talchain.util.ByteUtil.toHexString;
...
public class TransactionExecutor {
    ...
    public TransactionExecutor(Transaction tx, byte[] coinbase, Repository track, BlockStore blockStore,
        ...
        this.m_endGas = BIUtil.toBI(tx.getGasLimit());
    ...
    public void init() {
        ...
        if (cumulativeGasReached) {
            ...
            execError(String.format("Too much gas used in this block: Require: %s Got: %s", new BigInteger(1, currentBlock.getGasLimit()).longValue() - BIUtil.toBI(tx.getGasLimit()).longValue(), BIUtil.toBI(tx.getGasLimit()).longValue()));
        ...
        BigInteger txNonce = BIUtil.toBI(tx.getNonce());
        if (BIUtil.isNotEqual(reqNonce, txNonce)) {
            execError(String.format("Invalid nonce: required: %s , tx.nonce: %s", reqNonce, txNonce));

            return;
        }

        BigInteger txGasCost = BIUtil.toBI(tx.getGasPrice()).multiply(txGasLimit);
        BigInteger totalCost = BIUtil.toBI(tx.getValue()).add(txGasCost);
    ...
    public void execute() {
        ...
        if (!localCall) {
            ...
            BigInteger txGasLimit = BIUtil.toBI(tx.getGasLimit());
            BigInteger txGasCost = BIUtil.toBI(tx.getGasPrice()).multiply(txGasLimit);
            track.addBalance(tx.getSender(), txGasCost.negate());

            if (logger.isInfoEnabled())
                logger.info("Paying: txGasCost: [{}], gasPrice: [{}], gasLimit: [{}]", txGasCost, BIUtil.toBI(tx.getGasPrice()), txGasLimit);
        }
    ...
    private void call() {
        ...
        BigInteger endowment = BIUtil.toBI(tx.getValue());
...
//            }
        ...
        BigInteger endowment = BIUtil.toBI(tx.getValue());
        BIUtil.transfer(cacheTrack, tx.getSender(), newContractAddress, endowment);
    ...
    public void go() {
        ...
        try {
            ...
            if (vm != null) {
                ...
                m_endGas = BIUtil.toBI(tx.getGasLimit()).subtract(BIUtil.toBI(program.getResult().getGasUsed()));
...
//            https://github.com/ethereum/cpp-ethereum/blob/develop/libethereum/Executive.cpp#L241
    ...
    public TransactionExecutionSummary finalization() {
        ...
        if (result != null) {
            ...
            summaryBuilder
                    .gasUsed(BIUtil.toBI(result.getGasUsed()))
                    .gasRefund(BIUtil.toBI(gasRefund))
...
//            receipt.setPostTxState(track.getRoot()); // TODO later when RepositoryTrack.getRoot() is implemented
    ...
    public long getGasUsed() {
        return BIUtil.toBI(tx.getGasLimit()).subtract(m_endGas).longValue();
    }
