...
import org.talchain.crypto.ECIESCoder;
...
public class WhisperMessage extends ShhMessage {
    ...
    private void parse() {
        if (!parsed) {
            ...
            this.nonce = Util.rlpDecodeInt(paramsList.get(4));
...
//        setEncrypted(false);
        ...
        for (Topic kTopic : knownTopics) {
            for (int i = 0; i < topics.length; i++) {
                if (kTopic.equals(topics[i])) {
                    ...
                    byte[] gamma = HashUtil.sha3(ByteUtil.xor(kTopic.getFullTopic(), salt));
                    ECKey key = ECKey.fromPrivate(ByteUtil.xor(gamma, encryptedKey));
                    try {
    ...
    public byte[] hash() {
        return HashUtil.sha3(payload);
    }

    private int workProved() {
        byte[] d = new byte[64];
        System.arraycopy(HashUtil.sha3(encode(false)), 0, d, 0, 32);
        ByteBuffer.wrap(d).putInt(32, nonce);
        return getFirstBitSet(HashUtil.sha3(d));
    }
    ...
    private int seal(byte[] encoded, long pow) {
        ...
        System.arraycopy(HashUtil.sha3(encoded), 0, d, 0, 32);
        ...
        for (int bestBit = 0; System.currentTimeMillis() < then;) {
            for (int i = 0; i < 1024; ++i, ++nonce) {
                ...
                int fbs = getFirstBitSet(HashUtil.sha3(d));
                if (fbs > bestBit) {
                    ret = nonce;
                    bestBit = fbs;
                }
    ...
    public byte[] getBytes() {
        if (signature != null) {
            return ByteUtil.merge(new byte[]{flags}, payload, signature);
        } else {
            return ByteUtil.merge(new byte[]{flags}, payload);
        }
    }
    ...
    private void encrypt() {
        try {
            if (to != null) {
                ...
                for (int i = 0; i < topics.length; i++) {
                    rnd.nextBytes(salt);
                    byte[] gamma = HashUtil.sha3(ByteUtil.xor(topics[i].getFullTopic(), salt));
                    byte[] encodedKey = ByteUtil.xor(gamma, key.getPrivKeyBytes());
