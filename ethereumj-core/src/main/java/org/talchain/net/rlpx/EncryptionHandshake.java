...
public class EncryptionHandshake {
    ...
    public AuthInitiateMessage createAuthInitiate(@Nullable byte[] token, ECKey key) {
        ...
        message.ephemeralPublicHash = HashUtil.sha3(ephemeralKey.getPubKey(), 1, 64);
        message.publicKey = key.getPubKeyPoint();
        message.nonce = initiatorNonce;
        return message;
    }
    ...
    void agreeSecret(byte[] initiatePacket, byte[] responsePacket) {
        ...
        byte[] sharedSecret = HashUtil.sha3(agreedSecret, HashUtil.sha3(responderNonce, initiatorNonce));
        byte[] aesSecret = HashUtil.sha3(agreedSecret, sharedSecret);
        secrets = new Secrets();
        secrets.aes = aesSecret;
        secrets.mac = HashUtil.sha3(agreedSecret, aesSecret);
        secrets.token = HashUtil.sha3(sharedSecret);
