...
import org.talchain.config.NodeFilter;
import org.talchain.config.SystemProperties;
import org.talchain.core.Block;
import org.talchain.core.BlockWrapper;
            ...
            if(peer.isProtocolsInitialized()) {
                ...
                if (!activePeers.containsKey(peer.getNodeIdWrapper())) {
                    if (!peer.isActive() &&
                        ...
                        disconnect(peer, ReasonCode.TOO_MANY_PEERS);
                    } else {
                        process(peer);
                        addCnt++;
                    }
                } else {
                    disconnect(peer, ReasonCode.DUPLICATE_PEER);
                }
